...
# /creativestack:proposal-generator
...
- `case-studies/{slug}.md` (full case study files for the strongest 2-3 matches — long-form content, hero quote, metrics framings; from `/creativestack:case-study`)
- `learnings.md § Pitching` (per-sector win/loss patterns from past pitches; from `/creativestack:pitch-research` Log outcome mode)
- `team.md` (for team expertise, bios, and rates)
- `freelance-bench.md` (trusted freelance roster — used to credibly fill capacity gaps in scope/team sections; from `/creativestack:resource-conflict` Setup bench)
...
`/creativestack:setup` to configure your house visual style for sharper proposal
layouts."
...
- No project state for a returning client → "Run `/creativestack:project-kickoff` to formalise the relationship history first."
- No timeline → "Run `/creativestack:timeline-generator` to build phase structure with dependencies — proposals reference it directly."
- No competitive context → "Run `/creativestack:pitch-research` to strengthen the 'why us' section."
- No creative brief signal → "Run `/creativestack:creative-brief` to nail down objectives — proposals are stronger when the brief is solid."

### Downstream

- Client accepts → "Run `/creativestack:sow-generator` to formalise this into a Statement of Work, then `/creativestack:project-kickoff` to start delivery."
- Need a more detailed timeline before sending → "Run `/creativestack:timeline-generator` to build the full timeline with slip scenarios."
- Pitch lost or won → "Update `pitch_outcome` in Skill Memory and run `/creativestack:post-mortem` on the pitch — losses teach more than wins."
