...
This is the formal contract for calling `/source-scrape` from another skill (or directly from the user). Read this first when called by a caller skill.
...
> "I'll call `/source-scrape` with query='{the query}', mode='{mode}', output='{report|data}'."
...
> "I'll call `/source-scrape` with query='quiet luxury packaging 2026', mode='trends', output='data', time_window='last_6_months'."
...
> "I'll call `/source-scrape` with query='warm minimalism food brand identity', mode='visual', output='data', target_sources=8, max_per_source=4."
...
> "I'll call `/source-scrape` with query='Oatly recent campaigns 2025-2026', mode='brands', output='report', time_window='last_year'."
...
> "I'll call `/source-scrape` with query='Oatly creative leadership', mode='people', output='data'."
...
Caller skills should be aware: if `/source-scrape` was already run earlier in the conversation with the same query and mode, the second call **reuses the prior result**. This is intentional — avoids redundant scraping.
...
- Caller skills that depend heavily on visual evidence (`/design-research`, `/competitor-audit` in visual sections) should warn the user that visual evidence is text-only for this run
