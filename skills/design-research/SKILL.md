...
`/creativestack:source-scrape` for visual evidence, leans on serious designer
...
|---|---|---|
| `/creativestack:source-scrape` | Annotated visual captures, scored sources | Visual evidence, board references |
| `/creativestack:trend-report` | Visual trends, velocity scores, brand examples | Cultural territories, design movements |
| `/creativestack:competitor-audit` | Visual identity scores, category conventions | Visual landscape mode, adjacent examples |
| `/creativestack:creative-brief` | Objectives, audience, success criteria | Brief context — sharpens the research focus |
...
*"I can see you ran `/creativestack:source-scrape` and `/creativestack:trend-report`
...
> anyway, or run `/creativestack:brief-sharpener` first?"*
...
**Call `/creativestack:source-scrape` in visual mode** (any mode except Pattern
research, where Mobbin etc. are richer than scrape coverage):

> *"I'll call `/creativestack:source-scrape` with query='{key terms from the brief
...
> *"I'll call `/creativestack:source-scrape` with query='{topic} cultural context
...
- **Researching aesthetics before understanding the problem.** If you don't know
  ...
  Run `/creativestack:brief-sharpener` first when the brief is vague.
...
- No clear brief → "Run `/creativestack:brief-sharpener` first — the research will be 5x sharper with a tightened brief."
- No category context → "Run `/creativestack:competitor-audit` before this — the visual landscape mode will build on it."
- No trend signal → "Run `/creativestack:trend-report` first if you want to weight the research toward what's emerging."
- No visual evidence yet → "This skill calls `/creativestack:source-scrape` automatically, but you can run it standalone first if you want to inspect the captures."

### Downstream

- Strategic direction emerging → "Run `/creativestack:creative-strategy` to formalise the strategic territory these references point toward."
- Brief still feels off → "Run `/creativestack:brief-sharpener` to pressure-test the brief against this research context."
- Ready to brief the team → "Run `/creativestack:creative-brief` to translate the research into a brief the creative team can actually work from."
- Pitching this work → "Run `/creativestack:pitch-research` to pull in case studies that match the territories this research surfaced."

Pick the most relevant 1-2.

## Edge Cases

- **Very vague brief** — push back via the pre-flight check, surface the gaps, offer to run `/creativestack:brief-sharpener` first. Don't paper over a vague brief with broad research.
